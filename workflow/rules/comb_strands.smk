rule comb_strands_mutation:
    ...
    log:
        "logs/rule/analysis/{samples}/{samples}_{regions}_comb_strands_mutation.log",
    benchmark:
        "logs/rule/analysis/{samples}/{samples}_{regions}_comb_strands_mutation.benchmark.txt",
    shell:  
        """
...
rule comb_strands_mutation_intergenic:
    ...
    log:
        "logs/rule/analysis/{samples}/{samples}_{regions}_comb_strands_mutation_intergenic.log",
    benchmark:
        "logs/rule/analysis/{samples}/{samples}_{regions}_comb_strands_mutation_intergenic.benchmark.txt",
    shell:  
        """
        (echo "`date -R`: Combining plus stranded mutations..." &&
        awk '{{print $0"\\t""+"}}' {input.plus} > {output.comb} &&
