...
rep <- "_" # replicate
dprod <- "CPD" # damage product (CPD/64_PP)
...
# for plot B
df_rr_filt <- filter(df_rr_rs, phase != "async")
...
df_rr_ear_la$ear_la <- 0

df_rr_ear_la$ear_la[df_rr_ear_la$dataset_strand == "ERD"] <- 
  df_rr_ear_la$early[df_rr_ear_la$dataset_strand == "ERD"] / 
  df_rr_ear_la$late[df_rr_ear_la$dataset_strand == "ERD"]

df_rr_ear_la$ear_la[df_rr_ear_la$dataset_strand == "LRD"] <- 
  df_rr_ear_la$late[df_rr_ear_la$dataset_strand == "LRD"] / 
  df_rr_ear_la$early[df_rr_ear_la$dataset_strand == "LRD"]
...
stat.test = compare_means(ear_la ~ dataset_strand, data = pB_filt, method = "wilcox.test", 
...
p.A <- ggplot(pA_filt, aes(x = state_short, y = log2(xr_ds))) + 
  ...
  geom_boxplot(aes(fill=factor(states), linetype = dataset_strand), 
  ...
  scale_linetype_manual(name = "Replication Domains",
                        values=c("ERD"=1,"LRD"=2)) +
  guides(size = "none") 
...
p.B <- ggplot(pB_filt, aes(x = state_short, y = ear_la)) + 
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  geom_boxplot(aes(fill = factor(states), linetype=dataset_strand), outlier.shape = NA, lwd=0.5) +
  xlab(chrState_lab) + ylab("RR Relative to\nReplication Timing (log2)") +
  scale_fill_manual(name = "Chromatin States\n(#s in ERD/LRD)", values = state_colors) +
  scale_linetype_manual(name = "Replication Domains",
                        values=c("ERD"=1,"LRD"=2)) +
  guides(fill="none") 
...
#as.data.frame(table(pB_df_rr_ear_la$state_short[pB_df_rr_ear_la$dataset_strand == "ERD" & pB_df_rr_ear_la$sample_strand == "+"]))
#as.data.frame(table(pB_df_rr_ear_la$state_short[pB_df_rr_ear_la$dataset_strand == "LRD" & pB_df_rr_ear_la$sample_strand == "+"]))
