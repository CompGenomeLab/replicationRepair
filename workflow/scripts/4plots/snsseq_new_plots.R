...
real_name <- paste("/Users/azgarian/Documents/myprojects/replicationRepair/results/",
                   "final/final_reports_hg19_sns.seq.rep1.hela_",
                   "repdomains_no_overlap_windows_51_100_ready.csv",
                   sep = "")

sim_name <- paste("/Users/azgarian/Documents/myprojects/replicationRepair/results/",
                  "final/final_reports_sim_hg19_sns.seq.rep1.hela_repdomains",
                  "_no_overlap_windows_51_100_ready.csv", 
                  sep = "")

name="sns_seq_noverlap"
...
window <- window[ , c(1, 7, 2, 3, ncol(window))]
names(window) <- c("dataset","repdomains", "sample_names","sample_strand", "windows")
window$windows <- as.numeric(as.character(window$windows)) - 26
...
p_hela_real <- ggplot(real_df_org, aes(x = windows, y = RPKM)) + 
  ...
  xlab("Relative Position to SNS-seq (kb)") + ylab(fr_lab) +
...
p_hela_sim_rr <- ggplot(hela_sim_rr, aes(x = windows, y = log2(xr_ds))) + 
  ...
  geom_line(aes(color = sample_strand)) + 
  facet_grid(~product~time_after_exposure~phase~repdomains) + 
  xlab("Relative Position to SNS-seq (kb)") + ylab("Repair Rate (log2)") +
  scale_x_continuous(limits = c(-25, 25), 
                     breaks = c(-25, 0, 25), 
                     labels = c("-2.5", "0", "+2.5")) + 
  scale_color_manual(values = strand_colors) + 
...
p_df_rs <- ggplot(df_rs, aes(x = windows, y = log2(real_sim))) + 
  ...
  geom_line(aes(color = sample_strand)) + 
  facet_grid(~product~time_after_exposure~phase~method~repdomains) + 
  xlab("Relative Position to SNS-seq (kb)") + ylab("Real/Simulation (log2)") +
  scale_x_continuous(limits = c(-25, 25), 
                     breaks = c(-25, 0, 25), 
                     labels = c("-2.5", "0", "+2.5")) + 
  scale_color_manual(values = strand_colors) + 
...
p_df_rs <- ggplot(df_rr_rs, aes(x = windows, y = log2(real_sim))) + 
  ...
  geom_line(aes(color = sample_strand)) + 
  facet_grid(~product~time_after_exposure~phase~repdomains) + 
  xlab("Relative Position to SNS-seq (kb)") + ylab("Repair Rate Real/Simulation (log2)") +
  scale_x_continuous(limits = c(-25, 25), 
                     breaks = c(-25, 0, 25), 
                     labels = c("-2.5", "0", "+2.5")) + 
  scale_color_manual(values = strand_colors) + 
