...
# filtering for A.1
pA1_data <- filter(df_rr_org, phase != "async", replicate == rep, 
...
# filtering for B.1
pB1_data <- filter(df_rr_org, phase != "async", replicate == rep, 
...
p.A.1 + p.A.2.3 + grid::textGrob(paste('CPD\n12 min.\n', p_name, sep = ""), 
                                       rot = -90, gp=gpar(fontsize=12), 
                                       y = unit(.55, "npc")) + 
  p.B.1 + p.B.2.3 + grid::textGrob(paste('CPD\n120 min.\n', p_name, sep = ""), 
                                   rot = -90, gp=gpar(fontsize=12), 
                                   y = unit(.62, "npc")) + 
