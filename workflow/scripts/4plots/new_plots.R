...
real_name <- paste("/Users/azgarian/Documents/myprojects/replicationRepair/results/",
                   "final/final_reports_hg19_R21061297-EdUrep-UV3-5hrls_combined",
...
sim_name <- paste("/Users/azgarian/Documents/myprojects/replicationRepair/results/",
                  "final/final_reports_sim_hg19_R21061297-EdUrep-UV3-5hrls_combined",
                  "_hg19_peaks_repdomains_no_overlap_windows_51_100_ready.csv", 
                  sep = "")

name="edu_no_overlap_rep1_uv3-5h"
...
# filtering for A.1
real_df_org <- filter(real_df_org, product == "CPD", phase!= "async", replicate=="A")

sim_df_org <- filter(sim_df_org, product == "CPD", phase!= "async", replicate=="A")
...
p_hela_real <- ggplot(real_df_org, aes(x = windows, y = RPKM)) + 
  ...
  xlab("Relative Position to Edu Peaks (kb)") + ylab(fr_lab) +
...
p_df_rs <- ggplot(df_rs, aes(x = windows, y = log2(real_sim))) + 
  ...
  geom_line(aes(color = sample_strand)) + 
  facet_grid(~product~time_after_exposure~phase~method~repdomains) + 
  xlab("Relative Position to EdU Peaks (kb)") + ylab("Real/Simulation (log2)") +
  scale_x_continuous(limits = c(-25, 25), 
                     breaks = c(-25, 0, 25), 
                     labels = c("-2.5", "0", "+2.5")) + 
  scale_color_manual(values = strand_colors) + 
...
p_df_rs <- ggplot(df_rr_rs, aes(x = windows, y = log2(real_sim))) + 
  ...
  geom_line(aes(color = sample_strand)) + 
  facet_grid(~product~time_after_exposure~phase~repdomains) + 
  xlab("Relative Position to EdU Peaks (kb)") + ylab("Repair Rate Real/Simulation (log2)") +
  scale_x_continuous(limits = c(-25, 25), 
                     breaks = c(-25, 0, 25), 
                     labels = c("-2.5", "0", "+2.5")) + 
  scale_color_manual(values = strand_colors) + 
