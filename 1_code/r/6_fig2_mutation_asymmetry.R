...
fr_name <- "inzones_repdomains_201_100_melanoma_mutations_combined"
...
p.A.1.2 <- ggplot(data = mut_agg, aes(x = Group.2, y = x, 
                                      fill = Group.3)) + 
  geom_bar(stat = "identity", size = 1.5, 
           position=position_dodge2(padding = 0.05)) +
  facet_wrap(~direction) +
  #geom_text(aes(label = x), nudge_x = 0, color = "black") +
  ylim(0, 0.03) +
...
p.A.1.3 <- ggplot(data = mut_agg, aes(x = Group.2, y = log2(x))) + 
  geom_bar(stat = "identity", position=position_dodge()) +
  #geom_text(aes(label = x), nudge_y = 2, color = "black") +
  facet_wrap(~direction) +
  ylim(-0.3, 0.3) +
...
date <- "[2020.04.29.20:12]"

fr_name <- "inzones_repdomains_201_1000_melanoma_mutations_combined"
...
(p.A.1.1 + p.A.1.2 / p.A.1.3) / (p.B.1.1 + (p.B.1.2 / p.B.1.3)) +
  plot_annotation(tag_levels = c('A'), tag_suffix = ':') +
  plot_layout(guides = "collect", tag_level = 'new') & 
  theme(legend.position = 'bottom') 
  

ggsave("~/Desktop/fig2_new.png", width = 297, height = 210, units = "mm")
