...
fr_name <- "sns.seq.hela_windows_201_100"
date <- "[2020.05.06]"
...
# name of the script
figureInfo <- "figure/supp_sns_seq.R" 
...
rearrange <- function( ){
  window_number <- as.numeric(get_sub_str( fr_name, "windows_", "_" ))
...
# filter the data
d <- filter(df, phase != "async", method != "DNA_seq", replicate == repchoice)
