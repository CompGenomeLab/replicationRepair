...
figurePath <- paste("~/Documents/myprojects/replicationRepair/4_output/", 
                    "gitignore/Repdomains/", sep = "") 

# file name of data information
dataInfo <- paste(date, "final_report_", fr_name, "_info.TXT", sep = "") 

# name of the script
figureInfo <- "figure/supp_repdomains.R" 

# name of the region
region <- "Replication Domain"
...
d$dataset <- factor(d$dataset, levels = c("UTZ", "ERD", "DTZ", "LRD"))
...
# filter the data
d <- filter(df, replicate == repchoice, phase != "async")
...
# filter the data
d <- filter(df, replicate == repchoice, method != "DNA_seq", phase != "async")
