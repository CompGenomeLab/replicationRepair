...
p.A.1.2 <- p.A.1.2 + p_format + 
  theme(axis.title.x=element_blank(),
        ...
        axis.ticks.x = element_blank()) # add plot format
...
p.B.1.1 <- ggplot(mut) + 
  ...
  scale_color_manual(values = strand_colors) + 
  labs(color = "Strands") 
  #labs(subtitle = paste("200 kb sliding windows at 1000 bp intervals", sep = "")) 
...
(p.A.2.1 + p.A.2.2 + p.A.2.3 + p.A.1.1 + p.A.1.2 + p.A.1.3 + p.B.1.1 + 
    p.B.1.2 + p.B.1.3) +
...
ggsave("~/Desktop/fig3_manu.png", width = 297, height = 270, units = "mm")
